    ...
    return (
        <div className='w-full h-screen grid grid-cols-3 gap-4'>
            <div className='col-span-2 aspect-video rounded-xl truncate'>
                <iframe className='h-full w-full' height="100%" width="100%" src={activeURL}
                    title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                    referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
            </div>
            <div className='col-span-1 rounded-xl border-[2px] border-t-0 border-gray-100/80'>
                            ...
                            <ul className="">
                                {chapter.lessons?.map((lesson, lessonIndex) => (
                                    <li
                                        ...
                                        className={`cursor-pointer p-4 border-b hover:text-primary ${lesson.url === activeURL ? 'bg-gray-200 text-black rounded-none' : ''}`}
                                    >
