...
const EditNotificationDialog = ({
    ...
    const handleSubmit = async (e: any) => {
        ...
        const res = await dispatch(globalThis.$action.updateNotification(formatNotification))
        console.log(res)
        if (!res.error) {
            reload()
            close()
    ...
    useEffect(() => {
        if (!data) return
        const { title, description, groupId, status, _id } = data
        setNotification({ title, description, groupId, status, _id })
    }, [data])
