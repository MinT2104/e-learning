...
import { ImportGroupStudent } from './ImportGroupStudent'

const GroupStudentImportDialog = ({
    // className,
    reload,
    data,
    close
    ...
    return (
        <Dialog open={open} onOpenChange={handleChangeOpen}>
            <DialogContent className="max-w-[360px] md:max-w-lg flex flex-col gap-4 rounded-[20px] px-[30px] z-[9999] max-h-[70%] overflow-y-auto p-0">
                <ImportGroupStudent handleChangeOpen={handleChangeOpen} />
            </DialogContent>
        </Dialog>
    )
}

export default GroupStudentImportDialog